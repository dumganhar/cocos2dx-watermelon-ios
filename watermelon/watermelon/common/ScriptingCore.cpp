...
ccb_proxy_t *ccbuilder_proxy_ht = NULL;
...
#endif
//#define JS_NewCompartmentAndGlobalObject(cx, clasp, ppals) JS_NewGlobalObject(cx, clasp)
#endif
...
void ScriptingCore::setCCBProxy(const char *id, CCObject *obj) {
    ccb_proxy_t *t;
    CCBUILDER_NEW_PROXY(t, obj, id);
    ccb_proxy_t *l;
    CCBUILDER_GET_PROXY(l, obj);
    js_log("%s: %d ",l->id, obj);
}

const char* ScriptingCore::getCCBProxy(CCObject *obj) {
    ccb_proxy_t *t;
    CCBUILDER_GET_PROXY(t, obj);
    js_log("%s: %d ",t->id, obj);
    if(NULL == t || NULL == t->id) return NULL;
    return t->id;
}


int ScriptingCore::executeCCBuilderCallback(const char *id) {
    if(NULL == id) {
        return 0;
    }
    else {
        jsval dataVal = INT_TO_JSVAL(1);
        jsval retval;
        executeJSFunctionWithName(this->cx, global, id, dataVal, retval);
        return 1;
    }
...
int ScriptingCore::executeCCBuilderCallback(const char *id, jsval owner) {
    if(NULL == id) {
        return 0;
    }
    else {
        ...
        if(!JSVAL_IS_NULL(owner)) {
            js_log("%s: + ", JSVAL_TO_OBJECT(owner));
            executeJSFunctionWithName(this->cx, JSVAL_TO_OBJECT(owner), id, dataVal, retval);
        }
        return 1;
    }
...
int ScriptingCore::executeCCBuilderCallback(CCObject *self) {
    const char* id = getCCBProxy(self);
    if(NULL == id) {
        return 0;
    }
    else {

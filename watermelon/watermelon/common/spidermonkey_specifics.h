...
typedef struct ccb_proxy {
	void *ptr;
	char id[40];
	UT_hash_handle hh;
} ccb_proxy_t;

extern ccb_proxy_t *ccbuilder_proxy_ht;
...
#define CCBUILDER_NEW_PROXY(p, native_obj, id) \
do { \
p = (ccb_proxy_t *)malloc(sizeof(ccb_proxy_t)); \
assert(p); \
p->ptr = native_obj; \
strcpy(p->id, id);\
HASH_ADD_PTR(ccbuilder_proxy_ht, ptr, p); \
} while(0) \

#define CCBUILDER_GET_PROXY(p, native_obj) \
do { \
HASH_FIND_PTR(ccbuilder_proxy_ht, &native_obj, p); \
} while (0)

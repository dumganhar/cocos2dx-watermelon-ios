...
void dumpNamedRoots(const char *name, void *addr, JSGCRootType type, void *whoknows) {
    //js_log("Root: %s, %d", name, addr);
}

JSBool js_dumpRoot(JSContext *cx, uint32_t argc, jsval *vp) {
    JSRuntime *rt = JS_GetRuntime(cx);
    JS_DumpNamedRoots(rt, dumpNamedRoots, NULL);
    return JS_TRUE;
}


void ScriptingCore::dumpRoots() {
    JSContext *cx = ScriptingCore::getInstance()->getGlobalContext();
    js_dumpRoot(cx, 0, NULL);
}
...
void register_cocos2dx_js_extensions(JSContext* cx, JSObject* global)
{
    ...
    tmpObj = JSVAL_TO_OBJECT(anonEvaluate(cx, global, "(function () { return this; })()"));
    JS_DefineFunction(cx, tmpObj, "dumpRoot", js_dumpRoot, 1, JSPROP_READONLY | JSPROP_PERMANENT);
